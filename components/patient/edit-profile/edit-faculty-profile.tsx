...
import { updateFacultyProfile } from "@/actions/update-faculty-profile";
...
const EditFacultyProfile = ({ user, patient, info }: any) => {
	...
	const form = useForm<z.infer<typeof FacultyPatientSchema>>({
	...
	const onSubmit = (values: z.infer<typeof FacultyPatientSchema>) => {
		setError("");
		setSuccess("");

		startTransition(() => {
			updateFacultyProfile(patient.id, values).then((data: any) => {
				setError(data.Error);
					...
					<Form {...form}>
						<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
							<div className="space-y-3">
								...
								<FormLabel>Faculty Information</FormLabel>
								<FormField
									control={form.control}
									name="position"
									render={({ field }) => (
